...
I originally built this website with the idea of using TypeScript to build a website from scratch, and I set up the build utilizing Gulp.js and webpack, however, due to my neglect the current system is quite outdated and broken (from the standpoint of not being able to easily progress if forward).

After my adventures of deep-diving into all things Emacs related, and presented the /necessity/ to update my much outdated website, this led me to do so using Emacs' infamous Org Mode.

This is work is still in the very early stage of development, so there are many things to figure out - such as the exact type of site I want to build.
